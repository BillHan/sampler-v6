...
public class MenuBarHiddenItemsExample extends VerticalLayout {

    private MenuBar menubar = new MenuBar();

    public MenuBarHiddenItemsExample() {
        ...
        final MenuBar.MenuItem redo = edit.addItem("Redo", menuCommand);
        ...
        addComponent(new Button("Hide File menu", new Button.ClickListener() {
            public void buttonClick(ClickEvent event) {
                file.setVisible(!file.isVisible());
                event.getButton().setCaption(
                        file.isVisible() ? "Hide File menu" : "Show File menu");
            }
        }));

        addComponent(new Button("Enable Edit -> Redo action",
                new Button.ClickListener() {
                    public void buttonClick(ClickEvent event) {
                        redo.setEnabled(!redo.isEnabled());
                        event.getButton()
                                .setCaption(
                                        redo.isEnabled() ? "Disable Edit -> Redo action"
                                                : "Enable Edit -> Redo action");
                    }
                }));
